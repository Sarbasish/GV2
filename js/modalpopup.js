    ...
    $.fn.modalWindow = function(options){
		...
		$(window).resize(function(){
			$(options.modalContent).css({
				// top:$(window).height() /4 - currentModal.outerHeight() /4 + $(window).scrollTop(),
                // left:($(window).width() /4 - currentModal.outerWidth() /4 + $(window).scrollLeft()),
                // top: -130,
                // left:
			});
		});
		...
		function scrollModal(){
			$(window).scroll('on',function(){
				$(options.modalContent).css('position','relative');
			});
		}
